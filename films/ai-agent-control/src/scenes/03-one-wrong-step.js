// STUB
// Placeholder for shot 03 'one-wrong-step' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'one-wrong-step',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('one-wrong-step');
    L.paper(ctx);
    ...
    L.text(ctx, info.shot.title || 'one-wrong-step', 540, 1420, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'one-wrong-step', 540, 1480, { size: 30, align: 'center', color: P.inkSoft });
